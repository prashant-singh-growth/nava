...
export const IndustriesMenu = ({
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      switch (event.key) {
        ...
        case "ArrowDown":
          ...
          setFocusedIndex((prev) => (prev + 1) % industryItems.length);
          break;
        case "ArrowUp":
          event.preventDefault();
          setFocusedIndex((prev) => (prev - 1 + industryItems.length) % industryItems.length);
          break;
        case "Home":
          event.preventDefault();
          setFocusedIndex(0);
        ...
        case "End":
          event.preventDefault();
          setFocusedIndex(industryItems.length - 1);
          break;
        case "Enter":
        case " ":
          event.preventDefault();
          if (focusedIndex >= 0) {
            handleItemClick(industryItems[focusedIndex].route);
          }
          break;
      }
    };
                ...
                {industryItems.map((item, index) => (
                  <div key={item.id}>
                    <button
                      ...
                      className="w-full flex items-center px-4 py-3 hover:bg-[#F7FAFF] transition-all duration-200 text-left group"
                      role="menuitem"
                    >
                      <span className="text-black opacity-100 font-poppins font-normal text-[12px] leading-[27px] tracking-[0] flex-1 text-left">
                        {item.title}
                      </span>
                      <span className="text-black opacity-100">&gt;</span>
                    </button>
                    {index < industryItems.length - 1 && <div className="h-px bg-[#EFEFF5] mx-4"></div>}
                  </div>
                ))}
              </div>
            </div>
  ...
  return (
    <div ref={menuRef} className="absolute top-full left-1/2 transform -translate-x-1/2 z-50 ">
      <div
        className={cn(
          "bg-white rounded-3xl min-w-[395px]",
          "animate-in fade-in-0 slide-in-from-top-1 duration-[180ms] ease-out shadow-[0_3px_12px_#00000029]",
        )}
        role="menu"
          ...
          <div className="space-y-0">
            {industryItems.map((item, index) => {
              ...
              return (
                <div key={item.id}>
                  <button
                    onClick={() => handleItemClick(item.route)} onMouseEnter={() => { prefetchRoute(item.route); setFocusedIndex(index); }} onFocus={() => prefetchRoute(item.route)}
                    className={cn(
